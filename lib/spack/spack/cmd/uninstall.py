...
def installed_dependents(specs, env):
    ...
    Returns:
        (tuple): mapping from spec to sets of dependent specs
        ...
        for dpt in installed:
            if dpt not in specs:
                if not env or dpt.dag_hash() in env_hashes:
                    local_dpts.setdefault(spec, set()).add(dpt)
                else:
                    external_dpts.setdefault(spec, set()).add(dpt)

    return local_dpts, external_dpts
...
def inactive_dependent_environments(spec_envs):
    ...
    environment from all mappings.  Remvoe any specs in the map that now
...
def get_uninstall_list(args, specs, env):
    ...
    # Takes care of '-R'
    local_dpts, external_dpts = installed_dependents(uninstall_list, env)
    ...
    if has_error:
        ...
        for i, spec in enumerate(sorted(specs)):
            ...
            dependents = local_dpts.get(spec)
            if dependents:
